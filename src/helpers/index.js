...
export function getPoolLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://www.minidex.com/add/${token0Address}-${token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
        ? 'ETH'
        : '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
      }`
  } else {
    return `https://www.minidex.com/add/${token0Address}-${token1Address}`
  }
}

export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://www.minidex.com/swap?inputCurrency=${token0Address}`
  } else {
    return `https://www.minidex.com/swap?inputCurrency=${token0Address}&outputCurrency=${token1Address}`
  }
}
